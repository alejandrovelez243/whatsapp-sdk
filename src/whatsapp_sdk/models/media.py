...
class MediaURLResponse(BaseModel):
    ...
    url: str = Field(
        ...,
        description="URL to download the media (expires after 5 minutes)"
    )
    mime_type: str = Field(
        ...,
...
class MediaUploadRequest(BaseModel):
    ...
    filename: Optional[str] = Field(
        None,
        description="Filename to use (required when uploading bytes)"
    )
...
class SupportedMediaTypes(BaseModel):
    ...
    class DocumentTypes(BaseModel):
        ...
        docx: str = Field(default="application/vnd.openxmlformats-officedocument.wordprocessingml.document", description="Word documents")
        ppt: str = Field(default="application/vnd.ms-powerpoint", description="PowerPoint")
        pptx: str = Field(default="application/vnd.openxmlformats-officedocument.presentationml.presentation", description="PowerPoint")
        xls: str = Field(default="application/vnd.ms-excel", description="Excel")
        xlsx: str = Field(default="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", description="Excel")
...
class ResumableUploadSession(BaseModel):
    ...
    upload_url: str = Field(
        ...,
        description="URL for uploading file chunks"
    )
