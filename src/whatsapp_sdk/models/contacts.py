...
class Name(BaseModel):
    ...
    prefix: Optional[str] = Field(
        None,
        description="Name prefix (e.g., Mr., Ms., Dr.)"
    )
...
class Phone(BaseModel):
    ...
    phone: str = Field(
        ...,
        description="Phone number (can include formatting)"
    )
    type: Optional[str] = Field(
        None,
        description="Phone type: HOME, WORK, MOBILE, etc."
    )
    wa_id: Optional[str] = Field(
        None,
        description="WhatsApp ID if this number is on WhatsApp"
    )
...
class Url(BaseModel):
    ...
    type: Optional[str] = Field(
        None,
        description="URL type: HOME, WORK, etc."
    )
...
class Contact(BaseModel):
    ...
    org: Optional[Organization] = Field(
        None,
        description="Organization/workplace information"
    )
...
class ContactMessage(BaseModel):
    ...
    contacts: List[Contact] = Field(
        ...,
        min_length=1,
        description="List of contacts to send (at least 1)"
    )
...
class ReceivedContact(BaseModel):
    ...
    addresses: Optional[List[Address]] = Field(
        None,
        description="Contact physical addresses"
    )
    org: Optional[Organization] = Field(
        None,
